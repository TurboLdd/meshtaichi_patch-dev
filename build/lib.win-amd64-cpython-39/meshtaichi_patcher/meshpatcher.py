...
class MeshPatcher:#可能在这里面EE关系没有计算好
    def __init__(self, mesh=None):
        ...
        if self.face is not None:
            #print('face rel')no problem
            #print(self.face)
            self.patcher.set_relation(4, -1, Relation(self.face).csr)
            #四面体不能使用set_relation(2, 0, Relation(self.face).csr)
            #六面体不用set_relation(2, 0, Relation(self.face).csr)的话，会出现错误，3-2的关系没有计算好
            self.patcher.set_relation(4, 0, Relation(self.face).csr)
    ...
    def patch(self, max_order, patch_relation):
        ...
        for i in range(self.n_order):
            for j in range(self.n_order):
                ...
                print(f"{i}-{j}")
                self.get_relation(i,j).print()
        
        if max_order != -1:
            self.patcher.n_order = max_order
            self.n_order = max_order
        if patch_relation == 'all':
            print(max_order, self.n_order,self.patcher.n_order)
