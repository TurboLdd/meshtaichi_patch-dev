...
Csr& Patcher::get_relation(int from_end, int to_end) {
    ...
    auto &from = relation[{from_end, 0}];//3-0 c-v
    offset.push_back(0);
    for (int u = 0; u < from.size(); u++) {
        ...
        fun(0);
        offset.push_back(value.size());//输入尾部
    }
...
void Patcher::generate_elements() {
    ...
    get_relation(0, n_order - 1);//此时relation[(3,0)]=relation[(0,3)].T，即ele2vert
    
    auto &rel = relation[{n_order - 1, 0}];//ele2vert,rel即(3,0)
...
void Patcher::patch() {// the relation incorrect
    ...
    print_timer("cluster");
    start_timer("owned");//define 3-2 3-1 3-0
    ...
    std::cout<<"---------------1stget_relation(3,3) to see where getrelation used--------------"<<std::endl;
    auto &rel = get_relation(5,4 );
    rel.print();
    std::cout<<"---------------2ndget_relation(3,3) to see where getrelation used--------------"<<std::endl;
    auto &rel1 = get_relation(5, 4);//3-2 only 3 per elem.3-3 is none
    rel1.print();
    

    for (int order = 0; order < n_order - 1; order++) {//order:0-2
        auto &rel = get_relation(n_order - 1, order);//{3}{0}-{1}-{2}
...
Csr &Patcher::get_relation_meta(int from_end, int to_end) {
    using namespace std;
    std::cout<<"get_relation_meta.to see where getrelation used"<<std::endl;
