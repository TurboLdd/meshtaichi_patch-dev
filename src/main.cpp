...
//hex patch
template <typename T>
py::array_t<T> read_hexgen(std::string fn) {

    using namespace std;
    std::cout<<"read_hexgen"<<std::endl;
    fstream in(fn, fstream::in);
...
PYBIND11_MODULE(meshtaichi_patcher_core, m) {
    ...
    m.def("read_hexgen", [](std::string fn) {
        py::list ans;
        
        if (fn.substr(fn.size() - 4) == "node") ans.append(read_hexgen<float>(fn));
        else ans.append(read_hexgen<int>(fn));
        return ans;
    });
}
