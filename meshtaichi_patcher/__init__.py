...
from meshtaichi_patcher_core import read_hexgen
...
def load_mesh(meshes, relations=[], 
    ...
    """Create a triangle mesh (a set of vert/edge/face elements) \
       ...
       or a hexahedron mesh (a set of vert/edge/face/cell elements), \
...
def mesh2meta(meshes, relations=[], patch_size=256, cache=False, cluster_option="greedy", max_order=-1, refresh_cache=False, patch_relation="all", debug=False):
    ...
    print(max_order)
    m.patch(max_order, patch_relation)#这里是关键
...
def patched_mesh(filename):
    mesh = ti.TetMesh()#暂时未调用patched_mesh,先不修改
...
def load_mesh_rawdata(filename):
    ...
    if ext_name in ['node', 'ele']:
        ...
        #hexgen hardcoding
        if "hex" in base_name:
            ans[0] = read_hexgen(f'{base_name}.node')[0].reshape(-1, 3)
            ans[5] = read_hexgen(f'{base_name}.ele')[0].reshape(-1, 8)
            ans["face"] = read_hexgen(f'{base_name}.face')[0].reshape(-1, 4)#需要进一步确定
        else:
